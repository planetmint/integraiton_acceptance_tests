...
def test_get_tests():
    ...
    hostname = os.environ.get("PLANETMINT_ENDPOINT")
    bdb = Planetmint(hostname)
    ...
    prepared_transfer_tx = bdb.transactions.prepare(
        operation="TRANSFER", assets=[transfer_asset], inputs=transfer_input, recipients=bob.public_key
    )
    ...
    asset_id = bike_id
    url = "http://" + hostname + ":9984/api/v1/transactions?asset_ids=" + asset_id
    ...
    url = "http://" + hostname + ":9984/api/v1/transactions?asset_ids=" + asset_id + "&operation=CREATE"
    r = http.request("GET", url)
    tmp_json = http.request("GET", url)
    tmp_json = json.loads(tmp_json.data.decode("utf-8"))
    ...
    url = "http://" + hostname + ":9984/api/v1/transactions?asset_ids=" + asset_id + "&operation=transfer"
    r = http.request("GET", url)
    tmp_json = http.request("GET", url)
    tmp_json = json.loads(tmp_json.data.decode("utf-8"))
